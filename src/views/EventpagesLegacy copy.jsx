...
export default function EventpagesLegacy() {
  ...
  const handleNext = () => {
    setStartIndex((value) =>
      value + TABS_VISIBLE < categories.length ? value + 1 : value,
    );
  };
    ...
    <section className="relative mt-20 min-h-screen overflow-hidden bg-black px-4 py-16 text-white sm:px-6 md:px-12">
      <div className="mx-auto max-w-6xl space-y-10">
        <div className="flex justify-end text-xs font-bold tracking-wide text-white sm:text-sm">
          Total Events:{" "}
          <span className="ml-2 text-primary">{events.length}</span>
        </div>
        ...
        <div className="mb-10 flex items-center justify-center gap-8 font-asgard text-sm font-bold uppercase sm:text-base">
                ...
                <div className="relative flex h-20 w-20 items-center justify-center">
                  ...
                  >
                    <span className="inline-flex items-center gap-2">
                      {React.createElement(getTabIcon(category.name), {
                        className: "h-4 w-4",
                      })}
                      {category.name}
                    </span>
          ...
          <button
            ...
            className={`rounded p-1 ${
              startIndex + TABS_VISIBLE >= categories.length
        ...
        {events.map((event) => {
          ...
          return (
            <div
              key={event.id}
              className="border-b border-dashed border-gray-700 pb-6"
            >
